...
if (!is_array($data)) {
    ...
    exit('Ошибка декодирования JSON данных.');
}

$service = new TemplateService($conn);
$template = $service->getTemplateById((int)$row['template_id']);
...
if ($columnsCount === 0) {
    http_response_code(500);
    exit('В шаблоне нет заголовков колонок.');
}
...
$sheet->setCellValue($firstCol . '2', "МО: {$row['municipality_name']}   Дата: {$row['filled_date']}");
